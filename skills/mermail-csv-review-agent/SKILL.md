...
8. If requested, save one draft through `mermail-compose-email`, using the intended recipient supplied by the user, the selected thread context, and `body.body`. Include counts, unresolved decisions and a concise list of local output filenames. Do not claim files are attached unless a supported upload/attachment operation actually returned them on the saved draft.
